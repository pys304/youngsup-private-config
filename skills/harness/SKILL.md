---
name: harness
description: "도메인/프로젝트에 맞는 에이전트 팀과 스킬을 자동 생성하는 메타 스킬. '하네스 만들어줘', '에이전트 팀 구성해줘', 'build a harness', '스킬 생성해줘', '에이전트 설계해줘' 등 Claude Code용 에이전트/스킬/팀 아키텍처 구성을 요청할 때 반드시 이 스킬을 사용할 것. 기존 하네스 수정, 에이전트 추가, 스킬 업데이트, 운영/유지보수 요청에도 사용."
---
...
- 최종 산출물만 사용자 지정 경로에 출력, 중간 파일(`_workspace/`)은 보존
...
**CLAUDE.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. CLAUDE.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.

#### 5-5. 후속 작업 지원

오케스트레이터 description에 후속 키워드를 반드시 포함하라:
...
#### 6-3. 스킬 실행 테스트
1. 각 스킬에 대해 2~3개의 현실적인 테스트 프롬프트 작성
2. With-skill vs Without-skill(baseline) 비교 실행
3. assertion 기반 정량 평가 + 사용자 정성 평가
4. 피드백 일반화 후 스킬 수정 → 재테스트

#### 6-4. 트리거 검증
- **Should-trigger 쿼리** (8~10개) — 스킬을 트리거해야 하는 다양한 표현
- **Should-NOT-trigger 쿼리** (8~10개) — 키워드가 유사하지만 이 스킬이 아닌 경우 (near-miss)
...
#### 7-4. 진화 트리거
같은 유형의 피드백이 2회 이상 반복되거나, 에이전트가 반복적으로 실패하는 패턴이 발견될 때 진화를 제안한다.
...
- [ ] `프로젝트/.claude/agents/` — 에이전트 정의 파일 (빌트인 타입이라도 파일 생성 필수)
...
- [ ] 신규 에이전트/스킬 생성 전 중복 검토 완료 (Phase 3-0, 4-0)
- [ ] `.claude/commands/` — 아무것도 생성하지 않음
- [ ] 스킬 description이 pushy하게 작성됨 (후속 작업 키워드 포함)
